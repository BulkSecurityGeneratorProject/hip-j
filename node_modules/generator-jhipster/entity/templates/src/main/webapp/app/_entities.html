    ...
    if (fields[fieldId].fieldType == 'Integer' || fields[fieldId].fieldType == 'Long' || fields[fieldId].fieldType == 'BigDecimal') {
        ...
        fieldInputType = 'checkbox';
                        ...
                        <md-input-container flex>
                            ...
                            <input ng-model="<%= entityInstance %>.<%= fields[fieldId].fieldName %>">
                        </md-input-container><% } %>
...
<% for (relationshipId in relationships) {
    ...
    if (relationships[relationshipId].relationshipType == 'many-to-one') {
      ...
      <md-select placeholder="Placeholder"  name="<%= relationshipName %>" ng-model="<%=relationShipValue %>">
        <md-option ng-repeat="<%=otherEntityName %> in <%=otherEntityName.toLowerCase() %>s" value="{{<%=otherEntityName %>.id}}">{{<%=otherEntityName %>.<%=relationships[relationshipId].otherEntityField %>}}</md-option>
      </md-select>
